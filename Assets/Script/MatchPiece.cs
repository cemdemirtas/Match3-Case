...
namespace Match3
{
	public class MatchPiece : MonoBehaviour
	{
		...
		private void CheckDrag(Vector2 position)
		{
			...
			SwipeDirection dir = SwipeDirection.NULL;
			if (up) dir = SwipeDirection.UP;
			if (down) dir = SwipeDirection.DOWN;
			if (left) dir = SwipeDirection.LEFT;
			if (right) dir = SwipeDirection.RIGHT;

			if (dir != SwipeDirection.NULL)
